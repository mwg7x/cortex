...
import type { PrescriptionAnalysisRequest } from "@coretex/shared-types";
...
app.post("/api/v1/prescriptions/analyze", async (req, res) => {
  const payload = req.body as PrescriptionAnalysisRequest;
  ...
  const invalidMedication = payload.medications.find(
    (med) =>
  ...
  if (invalidMedication) {
    return res.status(400).json({
      error:
        "Each medication must include name, dose, and frequencyPerDay (1-4).",
    });
  }
...
function requireDoctor(req: any, res: any, next: any) {
  const token = req.headers["x-doctor-token"] as string | undefined;
...
app.get("/doctor/patients/:id/history", requireDoctor, (req, res) => {
  ...
  if (!history) return res.status(404).json({ error: "patient not found" });
...
app.post("/doctor/patients/:id/condition", requireDoctor, async (req, res) => {
  const { id } = req.params;
  const { condition, medications } = req.body as {
    condition?: string;
    medications?: any[];
  };
  ...
  try {
    ...
    // Run AI analysis
    const payload: PrescriptionAnalysisRequest = {
      diagnosis: condition,
      patient: {
        id: patient.id,
        name: patient.name,
...
app.post("/doctor/patients/:id/check", requireDoctor, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "analysis failed", detail: (err as Error).message });
  }
});
...
app.post("/api/v1/ask-ai", async (req, res) => {
  ...
  try {
    ...
    console.error("AI askGemini3 failed:", (err as Error).message);
